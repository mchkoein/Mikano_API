...
{
    [Authorize]
    [RoutePrefix("api/portfoliocorporatepagesections")]
    public class PortfolioCorporatePageSectionsController : SharedController<SocketHub>
    {
        private CorporatePageSectionRepository rpstry = new CorporatePageSectionRepository();

        private string kSectionName = "portfoliocorporatepagesections";
        ...
        public HttpResponseMessage GetAll([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, int parentId)
        {
            ...
            if (hasPermissions)
            {
                ...
                try
                {
                    ...
                    //var results = Enumerable.Empty<CorporatePageSection>().AsQueryable();
                    //CorporatePageSection parentEntry = rpstry.GetById(parentId);
                    ...
                    ////Getting all Corporate Page Sections of Parent Corporate Page Section
                    //if (parentEntry == null)
                    //{
                    //    results = rpstry.GetAll(parentId);

                    //}
                    ////Getting all Corporate Page Sections of Parent Corporate Page
                    //else
                    //{
                    //    results = rpstry.GetAll();
                    //}

                    //if (repeaterTemplateEntry != null)
                    //{
                    //    results = results.Where(d => d.subCorporatePageId == parentId);
                    //}
                    //else
                    //{
                    //    results = results.Where(d => d.subCorporatePageId == null);
                    //}

                    return Request.CreateResponse(HttpStatusCode.OK, rpstry.db.CorporatePageSections.Where(x => x.corporatePageId == 41 && x.portfolioId == parentId).Select(d => new
                    {
                        id = d.id,
                ...
                try
                {
                    ...
                    if (entry == null)
                    {
                        ...
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                corporatePageId = parentId, // repeaterTemplateEntry != null ? parentEntry.corporatePageId : parentId,
                                //subCorporatePageId = repeaterTemplateEntry != null ? parentEntry.id : (object)DBNull.Value,
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        if (entry.id == 0 && hasPermissionsToCreate)
                        {
                            ...
                            #endregion
                            entry.portfolioId = entry.corporatePageId;
                            entry.corporatePageId = 41;
        ...
        public string SortGrid(SortGridBindingModel model)
        {
            ...
            if (hasPermissions)
            {
                ...
                try
                {
                    ...
                    var result = rpstry.SortGrid(model.newIndex, model.oldIndex, model.id, model.specialParam, model.specialParamValue);
        ...
        [AllowAnonymous]
        [HttpGet]
        public HttpResponseMessage GetSectionDataById(int id, string imgsize = "", string imgsize1 = "", string imgsize2 = "")
        {
            ProjectKeysModel projectConfigKeys = new ProjectKeysHelper().GetKeys();
            ...
            var entrySubSectionsResults = new List<object>();
            ...
            foreach (var itemSubSections in entrySubSections)
            {
                ...
                entrySubSectionsResults.Add(new
                {
                    ...
                    #region Image (imgSrc)
                    smallImgSrc = GetResizedImagePath(projectConfigKeys: projectConfigKeys, imgSrc: itemSubSections.imgSrc, imgsize: imgsize1, imgDirectory: corporatePageSectionsDirectory, isProduct: false, isGiftCard: false),
                    largeImgSrc = GetResizedImagePath(projectConfigKeys, itemSubSections.imgSrc, imgsize2, corporatePageSectionsDirectory),
                    imgSrc = GetResizedImagePath(projectConfigKeys, itemSubSections.imgSrc, imgsize, corporatePageSectionsDirectory),
                    ...
                    smallImgSrcSecondary = GetResizedImagePath(projectConfigKeys: projectConfigKeys, imgSrc: itemSubSections.imgSrcSecondary, imgsize: imgsize1, imgDirectory: corporatePageSectionsDirectory, isProduct: false, isGiftCard: false),
                    largeImgSrcSecondary = GetResizedImagePath(projectConfigKeys, itemSubSections.imgSrcSecondary, imgsize2, corporatePageSectionsDirectory),
                    imgSrcSecondary = GetResizedImagePath(projectConfigKeys, itemSubSections.imgSrcSecondary, imgsize, corporatePageSectionsDirectory),
                    ...
                    videoUrl = itemSubSections.videoUrl,
                    videoSrc = string.IsNullOrEmpty(itemSubSections.videoSrc) ? "" : projectConfigKeys.apiUrl + "content/uploads/" + corporatePageSectionsDirectory + "/" + itemSubSections.videoSrc,
                    fileSrc = string.IsNullOrEmpty(itemSubSections.fileSrc) ? "" : projectConfigKeys.apiUrl + "content/uploads/" + corporatePageSectionsDirectory + "/" + itemSubSections.fileSrc,
                    link = subSectionsTranslatedItem == null ? itemSubSections.link : subSectionsTranslatedItem.link,
                    labelLink = subSectionsTranslatedItem == null ? itemSubSections.labelLink : subSectionsTranslatedItem.labelLink,

                    //smallImgSrc = GetResizedImagePath(projectConfigKeys, itemCollection.imgSrc, imgsize1, eKomCollectionsDirectory, false),
                    //largeImgSrc = GetResizedImagePath(projectConfigKeys, itemCollection.imgSrc, imgsize2, eKomCollectionsDirectory, false),
                    //imgSrc = GetResizedImagePath(projectConfigKeys, itemCollection.imgSrc, imgsize, eKomCollectionsDirectory, false),
                    urlTitle = !string.IsNullOrEmpty(itemSubSections.title) ? rpstry.GetUrlTitle(itemSubSections.title) : "",
                });
            }
            #endregion
            ...
            return Request.CreateResponse(HttpStatusCode.OK, new
            {
                ...
                #region Image (imgSrc)
                smallImgSrc = GetResizedImagePath(projectConfigKeys: projectConfigKeys, imgSrc: entry.imgSrc, imgsize: imgsize1, imgDirectory: corporatePageSectionsDirectory, isProduct: false, isGiftCard: false),
                largeImgSrc = GetResizedImagePath(projectConfigKeys, entry.imgSrc, imgsize2, corporatePageSectionsDirectory),
                imgSrc = GetResizedImagePath(projectConfigKeys, entry.imgSrc, imgsize, corporatePageSectionsDirectory),
                ...
                #region 2nd Image (imgSrcSecondary)
                smallImgSrcSecondary = GetResizedImagePath(projectConfigKeys: projectConfigKeys, imgSrc: entry.imgSrcSecondary, imgsize: imgsize1, imgDirectory: corporatePageSectionsDirectory, isProduct: false, isGiftCard: false),
                largeImgSrcSecondary = GetResizedImagePath(projectConfigKeys, entry.imgSrcSecondary, imgsize2, corporatePageSectionsDirectory),
                imgSrcSecondary = GetResizedImagePath(projectConfigKeys, entry.imgSrcSecondary, imgsize, corporatePageSectionsDirectory),
                ...
                videoSrc = string.IsNullOrEmpty(entry.videoSrc) ? "" : projectConfigKeys.apiUrl + "content/uploads/" + corporatePageSectionsDirectory + "/" + entry.videoSrc,
                fileSrc = string.IsNullOrEmpty(entry.fileSrc) ? "" : projectConfigKeys.apiUrl + "content/uploads/" + corporatePageSectionsDirectory + "/" + entry.fileSrc,
                link = entry.link,
                labelLink = entry.labelLink,

                #region Image Gallery 
                images = entry.CorporatePageSectionMedias.Where(e => e.mediaType == (int)MediaType.Image).Select(e => new
                {
                    smallImg = GetResizedImagePath(projectConfigKeys, e.mediaSrc, imgsize1, corporatePageSectionsDirectory),
                    largeImg = GetResizedImagePath(projectConfigKeys, e.mediaSrc, imgsize2, corporatePageSectionsDirectory),
                    imgSrc = GetResizedImagePath(projectConfigKeys, e.mediaSrc, imgsize, corporatePageSectionsDirectory),
                    caption = e.caption,
                ...
                #region Video Gallery 
                videos = entry.CorporatePageSectionMedias.Where(e => e.mediaType == (int)MediaType.Video).Select(e => new
                {
                    videoSrc = e.mediaSrc,
                    caption = e.caption,
                    subCaption = e.subCaption,
                ...
                #region File Gallery 
                files = entry.CorporatePageSectionMedias.Where(e => e.mediaType == (int)MediaType.File).Select(e => new
                {
                    fileSrc = e.mediaSrc,
                    caption = e.caption,
                    subCaption = e.subCaption,
