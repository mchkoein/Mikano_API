...
namespace Mikano_API.Controllers
{
    ...
    public class NewsCategoriesController : SharedController<SocketHub>
    {
        private NewsCategoryRepository rpstry = new NewsCategoryRepository();
        private string kSectionName = "NewsCategories";
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    ...
                    if (languageId.HasValue)
                    {
                        ...
                        entry = entry.NewsCategories.FirstOrDefault(d => !d.isDeleted && d.languageId == languageId);
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            ...
                            languages = languageRpstry.GetAllOptional().Select(e => new
                            {
                                ...
                                isFilled = entry.NewsCategories.Any(d => !d.isDeleted && d.languageId == e.id)
                            }),
                            additionalData = new
                            {
                                publishOptions = publishOptions,
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, NewsCategory entry, SubmissionOptions submissionType, bool inline = false)
        {
                        ...
                        else
                        {
                            ...
                            return Request.CreateResponse(HttpStatusCode.OK, new
                            {
                                ...
                                languages = languageRpstry.GetAllOptional().Select(e => new
                                {
                                    ...
                                    title = e.title,
                                    isRightToLeft = e.isRightToLeft,
                                    isFilled = (entry.NewsCategory1 ?? entry).NewsCategories.Any(d => !d.isDeleted && d.languageId == e.id)
                                })
                            });
                        }
                    }
            ...
            if (hasPermissions)
            {
                ...
                try
                {
                    ...
                    foreach (var parId in listOfIds)
                    {
                        ...
                        rpstry.Save();
                        parentEntry = entry.NewsCategory1;
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new
                    {
                        languages = parentEntry == null ? null : languageRpstry.GetAllOptional().Select(e => new
                        {
                            ...
                            title = e.title,
                            isRightToLeft = e.isRightToLeft,
                            isFilled = (parentEntry.NewsCategory1 ?? parentEntry).NewsCategories.Any(d => !d.isDeleted && d.languageId == e.id)
                        })
                    });
                }
                catch (Exception e)
        ...
        //{
        //    var NewsCategories = rpstry.GetAllIsPublished().Select(d => new
        //    {
        //        id = d.id,
        //        title = d.title,
        //        icon = d.icon,
        //        link = d.link
        //    });
        //    return Request.CreateResponse(HttpStatusCode.OK, NewsCategories);
        //}
        #endregion
    }
