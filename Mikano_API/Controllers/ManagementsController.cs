...
{
    [Authorize]
    [RoutePrefix("api/Managements")]
    public class ManagementsController : SharedController<SocketHub>
    {
        private ManagementRepository rpstry = new ManagementRepository();
        private string kSectionName = "managements";
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    ...
                    if (languageId.HasValue)
                    {
                        ...
                        if (entry != null)
                        {
                            entry = entry.Managements.FirstOrDefault(d => !d.isDeleted && d.languageId == languageId);
                        }
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                ...
                                imgSrc = new UploadController().GetUploadedFiles(entry.imgSrc, null, managementsDirectory),
                                email = entry.email,
                                phone = entry.phone,
                                mobile = entry.mobile,
                            ...
                            languages = languageRpstry.GetAllOptional().Select(e => new
                            {
                                ...
                                isFilled = entry.Managements.Any(d => !d.isDeleted && d.languageId == e.id)
                            }),
                            additionalData = new
                            {
                                publishOptions = publishOptions,
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, Management entry, SubmissionOptions submissionType, bool inline = false)
        {
            KMSSectionRepository sectionRpstry = new KMSSectionRepository();
            KMSLogRepository logRpstry = new KMSLogRepository();
            LanguageRepository languageRpstry = new LanguageRepository();
                        ...
                        else
                        {
                            return Request.CreateResponse(HttpStatusCode.OK, new
                            {
                                ...
                                languages = languageRpstry.GetAllOptional().Select(e => new
                                {
                                    ...
                                    title = e.title,
                                    isRightToLeft = e.isRightToLeft,
                                    isFilled = (entry.Management1 ?? entry).Managements.Any(d => !d.isDeleted && d.languageId == e.id)
                                })
                            });
                        }
                    }
                ...
                try
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new
                    {
                        languages = parentEntry == null ? null : languageRpstry.GetAllOptional().Select(e => new
                        {
                            ...
                            title = e.title,
                            isRightToLeft = e.isRightToLeft,
                            isFilled = (parentEntry.Management1 ?? parentEntry).Managements.Any(d => !d.isDeleted && d.languageId == e.id)
                        })
                    });
                }
                catch (Exception e)
        ...
        public HttpResponseMessage GetData()
        {
            ...
            {
                var entryTranslatedItem = requestedLanguageId == -1 ? null : entryItem.Managements.FirstOrDefault(lang => lang.languageId == requestedLanguageId);
