...
{
    [Authorize]
    [RoutePrefix("api/kconfigcategories")]
    public class KConfigCategoriesController : SharedController<SocketHub>
    {
        private KConfigCategoryRepository rpstry = new KConfigCategoryRepository();
        private string kSectionName = "kconfigcategories";
        ...
        public HttpResponseMessage GetById(int id, int? languageId = null)
        {
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    KConfigCategory entry = rpstry.GetById(id);
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, KConfigCategory entry, SubmissionOptions submissionType, bool inline = false)
        {
            ...
            if (hasPermissionsToCreate || hasPermissionsToUpdate)
            {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        #region Unpublish other entries with same title
                        if (entry.isPublished)
                        {
                            var oldPublishedEntry = rpstry.GetAll().FirstOrDefault(d => d.isPublished && d.id != entry.id && d.title.ToLower() == entry.title.ToLower());
                            if (oldPublishedEntry != null)
                            {
                                oldPublishedEntry.isPublished = false;
                            }
                        }
                        #endregion
