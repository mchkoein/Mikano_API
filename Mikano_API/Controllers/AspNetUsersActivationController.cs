...
{
    [Authorize]
    [RoutePrefix("api/AspNetUsersActivations")]
    public class AspNetUsersActivationsController : SharedController<SocketHub>
    {
        private AspNetUsersActivationRepository rpstry = new AspNetUsersActivationRepository();
        private string kSectionName = "aspnetusersactivations";
        ...
        [HttpGet]
        public HttpResponseMessage GetAllIsPublished([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request)
        {
            return Request.CreateResponse(HttpStatusCode.OK,
            rpstry.GetAllIsPublished().Select(d => new
            {
                id = d.id,
                title = d.title,
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, AspNetUsersActivation entry, SubmissionOptions submissionType, bool inline = false)
        {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        else if (hasPermissionsToUpdate)
                        {
                            ...
                            oldEntry.hasReason = entry.hasReason;
