...
namespace Mikano_API.Controllers
{
    ...
    public class ReportsController : SharedController<SocketHub>
    {
        private ReportRepository rpstry = new ReportRepository();
        private string kSectionName = "reports";
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    ...
                    if (languageId.HasValue)
                    {
                        ...
                        if (entry != null)
                        {
                            entry = entry.Reports.FirstOrDefault(d => !d.isDeleted && d.languageId == languageId);
                        }
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                ...
                                fileSrc = new UploadController().GetUploadedFiles(entry.fileSrc, null, reportsDirectory),
                            },
                            languages = languageRpstry.GetAllOptional().Select(e => new
                            {
                                ...
                                isFilled = entry.Reports.Any(d => !d.isDeleted && d.languageId == e.id)
                            }),
                            additionalData = new
                            {
                                publishOptions = publishOptions,
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, Report entry, SubmissionOptions submissionType, bool inline = false)
        {
            KMSSectionRepository sectionRpstry = new KMSSectionRepository();
            KMSLogRepository logRpstry = new KMSLogRepository();
            LanguageRepository languageRpstry = new LanguageRepository();
                        ...
                        else
                        {
                            return Request.CreateResponse(HttpStatusCode.OK, new
                            {
                                ...
                                languages = languageRpstry.GetAllOptional().Select(e => new
                                {
                                    ...
                                    title = e.title,
                                    isRightToLeft = e.isRightToLeft,
                                    isFilled = (entry.Report1 ?? entry).Reports.Any(d => !d.isDeleted && d.languageId == e.id)
                                })
                            });
                        }
                    }
                ...
                try
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new
                    {
                        languages = parentEntry == null ? null : languageRpstry.GetAllOptional().Select(e => new
                        {
                            ...
                            title = e.title,
                            isRightToLeft = e.isRightToLeft,
                            isFilled = (parentEntry.Report1 ?? parentEntry).Reports.Any(d => !d.isDeleted && d.languageId == e.id)
                        })
                    });
                }
                catch (Exception e)
        ...
        public HttpResponseMessage GetData()
        {
            ...
            {
                var entryTranslatedItem = requestedLanguageId == -1 ? null : entryItem.Reports.FirstOrDefault(lang => lang.languageId == requestedLanguageId);
