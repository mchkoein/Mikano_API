...
namespace Mikano_API.Controllers
{
    ...
    public class PartnersController : SharedController<SocketHub>
    {
        private PartnerRepository rpstry = new PartnerRepository();
        private string kSectionName = "Partners";
                ...
                try
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK,
                        rpstry.GetAll().Select(d => new
                        {
                            ...
                            link = d.link,
                            category = d.PartnerCategory.title,
                            priority = d.priority,
        ...
        public HttpResponseMessage GetById(int id)
        {
            KMSSectionRepository sectionRpstry = new KMSSectionRepository();
            KMSLogRepository logRpstry = new KMSLogRepository();
            PartnerCategoryRepository PartnerCategoryRepository = new PartnerCategoryRepository();
            ...
            var PartnerCategories = PartnerCategoryRepository.GetAllIsPublished().Select(d => new { id = d.id + "", label = d.title });
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                id = entry.id,
                                title = entry.title,
                                logoImage = new UploadController().GetUploadedFiles(entry.logoImage, null, partnersDirectory),
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))] DataSourceRequest request, Partner entry, SubmissionOptions submissionType, bool inline = false)
        {
            KMSSectionRepository sectionRpstry = new KMSSectionRepository();
            KMSLogRepository logRpstry = new KMSLogRepository();
            loggedInUserId = User.Identity.GetUserId();
