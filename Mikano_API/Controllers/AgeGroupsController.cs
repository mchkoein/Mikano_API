...
namespace Mikano_API.Controllers
{
    ...
    public class AgeGroupsController : SharedController<SocketHub>
    {
        private AgeGroupRepository rpstry = new AgeGroupRepository();
        private string kSectionName = "agegroups";
        ...
        public HttpResponseMessage GetById(int id, int? languageId = null)
        {
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    AgeGroup entry = rpstry.GetById(id);
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, AgeGroup entry, SubmissionOptions submissionType, bool inline = false)
        {
