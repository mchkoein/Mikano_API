...
{
    [Authorize]
    [RoutePrefix("api/AspNetUsersCategories")]
    public class AspNetUsersCategoriesController : SharedController<SocketHub>
    {
        private AspNetUsersCategoryRepository rpstry = new AspNetUsersCategoryRepository();
        private string kSectionName = "aspnetuserscategories";
                ...
                try
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK,
                        rpstry.GetAll().Select(d => new
                        {
                            id = d.id,
                            title = d.title,
                            //discount = d.EKomDiscount == null ? "" : d.EKomDiscount.title,
                            discountId = d.ekomDiscountId,
                            priority = d.priority,
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                id = entry.id,
                                title = entry.title,
                                ekomDiscountId = entry.ekomDiscountId + "",
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, AspNetUsersCategory entry, SubmissionOptions submissionType, bool inline = false)
        {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        else if (hasPermissionsToUpdate)
                        {
                            ...
                            oldEntry.ekomDiscountId = entry.ekomDiscountId;
