...
{
    [Authorize]
    [RoutePrefix("api/iconlists")]
    public class IconListsController : SharedController<SocketHub>
    {
        private IconListRepository rpstry = new IconListRepository();
        private string kSectionName = "iconlists";
        ...
        public HttpResponseMessage GetById(int id, int? languageId = null)
        {
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    IconList entry = rpstry.GetById(id);
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                id = entry.id,
                                title = entry.title,
                                imgListing = new UploadController().GetUploadedFiles(entry.imgListing,null,IconsDirectory),
                                imgSrc = new UploadController().GetUploadedFiles(entry.imgSrc, null, IconsDirectory),
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, IconList entry, SubmissionOptions submissionType, bool inline = false)
        {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        else if (hasPermissionsToUpdate)
                        {
                            ...
                            oldEntry.icon_class = entry.icon_class;
