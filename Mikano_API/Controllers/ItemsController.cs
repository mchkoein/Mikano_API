...
{
    [Authorize]
    [RoutePrefix("api/branditems")]
    public class BrandItemsController : SharedController<SocketHub>
    {
        private BrandItemRepository rpstry = new BrandItemRepository();
        private string kSectionName = "branditems";
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            model = new
                            {
                                ...
                                logoImage = new UploadController().GetUploadedFiles(entry.imgSrc, null, brandDirectory),
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))] DataSourceRequest request, BrandItem entry, SubmissionOptions submissionType, bool inline = false)
        {
            KMSSectionRepository sectionRpstry = new KMSSectionRepository();
            KMSLogRepository logRpstry = new KMSLogRepository();
