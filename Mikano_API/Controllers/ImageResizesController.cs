...
{
    [Authorize]
    [RoutePrefix("api/imageresizes")]
    public class ImageResizesController : SharedController<SocketHub>
    {
        private ImageResizeRepository rpstry = new ImageResizeRepository();
        private string kSectionName = "imageresizes";
        ...
        public HttpResponseMessage GetById(int id)
        {
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    ...
                    logRpstry.AddLog(loggedInUserId, User.Identity.Name, kActionName, kSectionName, id + "", entry == null ? kSectionName : entry.FullSize, CollectRequestData(Request, null), GetIpAddress(Request), false);
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, ImageResize entry, SubmissionOptions submissionType, bool inline = false)
        {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        else if (hasPermissionsToUpdate)
                        {
                            ...
                            oldEntry.width = entry.width;
                            oldEntry.height = entry.height;
                            oldEntry.section = entry.section;
                            oldEntry.isInside = entry.isInside;
                        ...
                        #region Resize Images of respective section
                        new UploadController().ResizeExistingImages(entry.section);
                        #endregion


                        logRpstry.AddLog(loggedInUserId, User.Identity.Name, kActionName, kSectionName, entry.id.ToString(), entry.FullSize, CollectRequestData(Request, entry), GetIpAddress(Request), false);

                        return Request.CreateResponse(HttpStatusCode.OK, new { id = entry.id });
                    }
                    catch (Exception e)
                    {
                        logRpstry.AddLog(loggedInUserId, User.Identity.Name, kActionName, kSectionName, entry.id.ToString(), entry.FullSize, CollectRequestData(Request, entry), GetIpAddress(Request), true);
            ...
            if (hasPermissions)
            {
                ...
                try
                {
                    ...
                    foreach (var parId in listOfIds)
                    {
                        ...
                        logRpstry.AddLog(loggedInUserId, User.Identity.Name, kActionName, kSectionName, parId, entry.FullSize, CollectRequestData(Request, null), GetIpAddress(Request), false);
