...
namespace Mikano_API.Controllers
{
    ...
    public class TagsController : SharedController<SocketHub>
    {
        private TagRepository rpstry = new TagRepository();
        private string kSectionName = "Tags";
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    ...
                    if (languageId.HasValue)
                    {
                        ...
                        entry = entry.Tags.FirstOrDefault(d => !d.isDeleted && d.languageId == languageId);
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            ...
                            languages = languageRpstry.GetAllOptional().Select(e => new
                            {
                                ...
                                isFilled = entry.Tags.Any(d => !d.isDeleted && d.languageId == e.id)
                            }),
                            additionalData = new
                            {
                                publishOptions = publishOptions,
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, Tag entry, SubmissionOptions submissionType, bool inline = false)
        {
                        ...
                        else
                        {
                            ...
                            return Request.CreateResponse(HttpStatusCode.OK, new
                            {
                                ...
                                languages = languageRpstry.GetAllOptional().Select(e => new
                                {
                                    ...
                                    title = e.title,
                                    isRightToLeft = e.isRightToLeft,
                                    isFilled = (entry.Tag1 ?? entry).Tags.Any(d => !d.isDeleted && d.languageId == e.id)
                                })
                            });
                        }
                    }
                ...
                try
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new
                    {
                        languages = parentEntry == null ? null : languageRpstry.GetAllOptional().Select(e => new
                        {
                            ...
                            title = e.title,
                            isRightToLeft = e.isRightToLeft,
                            isFilled = (parentEntry.Tag1 ?? parentEntry).Tags.Any(d => !d.isDeleted && d.languageId == e.id)
                        })
                    });
                }
                catch (Exception e)
        ...
        //{
        //    var Tags = rpstry.GetAllIsPublished().Select(d => new
        //    {
        //        id = d.id,
        //        title = d.title,
        //        icon = d.icon,
