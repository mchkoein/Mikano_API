...
namespace Mikano_API.Controllers
{
    ...
    public class PortfoliosController : SharedController<SocketHub>
    {
        private PortfolioRepository rpstry = new PortfolioRepository();
        private string kSectionName = "Portfolios";
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))] DataSourceRequest request, Portfolio entry, SubmissionOptions submissionType, bool inline = false)
        {
            KMSSectionRepository sectionRpstry = new KMSSectionRepository();
            KMSLogRepository logRpstry = new KMSLogRepository();
            ...
            if (hasPermissionsToCreate || hasPermissionsToUpdate)
            {
                ...
                if (ModelState.IsValid)
                {
                    try
                    {
                        ...
                        {
                            entry.CorporatePageSections.Add(new CorporatePageSection
                            {
                                corporatePageId = 41,
                                corporatePageTemplateId = 80,
