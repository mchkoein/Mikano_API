...
{
    [Authorize]
    [RoutePrefix("api/powercapacities")]
    public class PowerCapacitiesController : SharedController<SocketHub>
    {
        private PowerCapacityRepository rpstry = new PowerCapacityRepository();
        private string kSectionName = "powercapacities";
            ...
            if (hasPermissionsToCreate && id == -1 || hasPermissionsToUpdate && id != -1)
            {
                ...
                try
                {
                    ...
                    if (languageId.HasValue)
                    {
                        ...
                        entry = entry.PowerCapacities.FirstOrDefault(d => !d.isDeleted && d.languageId == languageId);
                    ...
                    else
                    {
                        return Request.CreateResponse(HttpStatusCode.OK, new
                        {
                            ...
                            languages = languageRpstry.GetAllOptional().Select(e => new
                            {
                                ...
                                isFilled = entry.PowerCapacities.Any(d => !d.isDeleted && d.languageId == e.id)
                            }),
                            additionalData = new
                            {
                                publishOptions = publishOptions,
        ...
        [HttpPost, HttpPut]
        public HttpResponseMessage Details([ModelBinder(typeof(WebApiDataSourceRequestModelBinder))]DataSourceRequest request, PowerCapacity entry, SubmissionOptions submissionType, bool inline = false)
        {
                        ...
                        else
                        {
                            ...
                            return Request.CreateResponse(HttpStatusCode.OK, new
                            {
                                ...
                                languages = languageRpstry.GetAllOptional().Select(e => new
                                {
                                    ...
                                    title = e.title,
                                    isRightToLeft = e.isRightToLeft,
                                    isFilled = (entry.PowerCapacity1 ?? entry).PowerCapacities.Any(d => !d.isDeleted && d.languageId == e.id)
                                })
                            });
                        }
                    }
        ...
        public HttpResponseMessage Delete(string ids = "")
        {
            ...
            if (hasPermissions)
            {
                ...
                try
                {
                    PowerCapacity parentEntry = null;
                    ...
                    foreach (var parId in listOfIds)
                    {
                        ...
                        rpstry.Save();
                        parentEntry = entry.PowerCapacity1;
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new
                    {
                        languages = parentEntry == null ? null : languageRpstry.GetAllOptional().Select(e => new
                        {
                            ...
                            title = e.title,
                            isRightToLeft = e.isRightToLeft,
                            isFilled = (parentEntry.PowerCapacity1 ?? parentEntry).PowerCapacities.Any(d => !d.isDeleted && d.languageId == e.id)
                        })
                    });
                }
                catch (Exception e)
        ...
        //{
        //    var PowerCapacities = rpstry.GetAllIsPublished().Select(d => new
        //    {
        //        id = d.id,
        //        title = d.title,
        //        icon = d.icon,
        //        link = d.link
        //    });
        //    return Request.CreateResponse(HttpStatusCode.OK, PowerCapacities);
        //}
        #endregion
    }
