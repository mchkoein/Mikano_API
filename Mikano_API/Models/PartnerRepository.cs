...
{
    #region Methods
    public class PartnerRepository : SharedRepository
    {
        public dblinqDataContext db = new dblinqDataContext();

        public IQueryable<Partner> GetAll()
        {
            return db.Partners.Where(d => !d.isDeleted).OrderByDescending(d => d.priority);
        }

        public IQueryable<Partner> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public Partner GetNextEntry(Partner entry)
        {
        ...
        {
            return db.Partners.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }
        ...
        {
            return db.Partners.Any() ? db.Partners.Max(d => d.priority) : 0;
        }

        public void Add(Partner entry)
        {
            db.Partners.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.Partners.Where(d => !d.isDeleted);
