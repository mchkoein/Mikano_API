...
namespace Mikano_API.Models
{
    ...
    public class ProductRepository : SharedRepository
    {
        ...
        {
            return db.Products.Where(d => !d.isDeleted).OrderByDescending(d => d.priority);
        }

        public IQueryable<Product> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public Product GetNextEntry(Product entry)
        {
        ...
        {
            return db.Products.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }

        //public Product GetByUrlTitle(string title)
        //{
        //    return db.Products.FirstOrDefault(d => !d.isDeleted && d.title.Replace("<br/>", "-").Replace("<br>", "-").Replace("!", "").Replace(":", "-").Replace("'", "").Replace(" ", "-").Replace("&", "").Replace("?", "").Replace("/", "-").Replace("*", "-").Replace(".", "").Replace("--", "-").ToLower() == title.ToLower());
        //}

        //public Product GetByNativeTitle(string title)
        //{
        //    return db.Products.FirstOrDefault(d => !d.isDeleted && d.title.ToLower() == title.ToLower());
        //}

        public int GetMaxPriority()
        {
            return db.Products.Any() ? db.Products.Max(d => d.priority) : 0;
        }

        public void Add(Product entry)
        {
            db.Products.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.Products.Where(d => !d.isDeleted);
        ...
        //{
        //    db.Products.DeleteOnSubmit(db.Products.FirstOrDefault(d=>d.id == id));
        //}
        ...
        public void DeleteRelatedMedias(Product entry)
        {
            db.ProductMedias.DeleteAllOnSubmit(db.ProductMedias.Where(d => d.product_id == entry.id));
        }
    }
    #endregion
}