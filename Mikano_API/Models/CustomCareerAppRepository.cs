...
{
    #region Models
    [MetadataType(typeof(CustomCareerAppValidation))]
    public partial class CustomCareerApp
    {
    ...
    public class CustomCareerAppValidation
    {
        //[Required]
    ...
    #region Methods
    public class CustomCareerAppRepository : SharedRepository
    {
        private dblinqDataContext db = new dblinqDataContext();

        public IQueryable<CustomCareerApp> GetAll()
        {
            return db.CustomCareerApps.Where(d => !d.isDeleted).OrderByDescending(d => d.dateCreated);
        }

        public CustomCareerApp GetNextEntry(CustomCareerApp entry)
        {
        ...
        public CustomCareerApp GetById(int id)
        {
        ...
        {
            return db.CustomCareerApps.Any() ? db.CustomCareerApps.Max(d => d.priority) : 0;
        }

        public void Add(CustomCareerApp entry)
        {
            db.CustomCareerApps.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.CustomCareerApps.Where(d => !d.isDeleted);
        ...
        //{
        //    db.CustomCareerApps.DeleteOnSubmit(db.CustomCareerApps.FirstOrDefault(d=>d.id == id));
        //}
