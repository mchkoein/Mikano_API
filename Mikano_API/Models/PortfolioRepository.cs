...
namespace Mikano_API.Models
{
    ...
    #region Methods
    public class PortfolioRepository : SharedRepository
    {
        public dblinqDataContext db = new dblinqDataContext();

        public IQueryable<Portfolio> GetAll()
        {
            return db.Portfolios.Where(d => !d.isDeleted).OrderByDescending(d => d.priority);
        }

        public IQueryable<Portfolio> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public Portfolio GetNextEntry(Portfolio entry)
        {
        ...
        {
            return db.Portfolios.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }

        //public Portfolio GetByUrlTitle(string title)
        //{
        //    return db.Portfolios.FirstOrDefault(d => !d.isDeleted && d.title.Replace("<br/>", "-").Replace("<br>", "-").Replace("!", "").Replace(":", "-").Replace("'", "").Replace(" ", "-").Replace("&", "").Replace("?", "").Replace("/", "-").Replace("*", "-").Replace(".", "").Replace("--", "-").ToLower() == title.ToLower());
        //}

        //public Portfolio GetByNativeTitle(string title)
        //{
        //    return db.Portfolios.FirstOrDefault(d => !d.isDeleted && d.title.ToLower() == title.ToLower());
        //}

        public int GetMaxPriority()
        {
            return db.Portfolios.Any() ? db.Portfolios.Max(d => d.priority) : 0;
        }

        public void Add(Portfolio entry)
        {
            db.Portfolios.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.Portfolios.Where(d => !d.isDeleted);
        ...
        //{
        //    db.Portfolios.DeleteOnSubmit(db.Portfolios.FirstOrDefault(d=>d.id == id));
        //}
