...
{
    #region Models
    [MetadataType(typeof(CareerValidation))]
    ...
    #region Methods
    public class CareerRepository : SharedRepository
    {
        public dblinqDataContext db = new dblinqDataContext();

        public IQueryable<Career> GetAll()
        {
            return db.Careers.Where(d => !d.isDeleted && d.languageParentId == null).OrderByDescending(d => d.priority);
        }

        public IQueryable<Career> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public Career GetNextEntry(Career entry)
        {
        ...
        {
            return db.Careers.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }

        public Career GetByUrlTitle(string title)
        {
            return db.Careers.FirstOrDefault(d => !d.isDeleted && d.title.Replace("<br/>", "-").Replace("<br>", "-").Replace("!", "").Replace(":", "-").Replace("'", "").Replace(" ", "-").Replace("&", "").Replace("?", "").Replace("/", "-").Replace("*", "-").Replace(".", "").Replace("--", "-").ToLower() == title.ToLower());
        }

        public Career GetByNativeTitle(string title)
        {
            return db.Careers.FirstOrDefault(d => !d.isDeleted && d.title.ToLower() == title.ToLower());
        }

        public int GetMaxPriority()
        {
            return db.Careers.Any() ? db.Careers.Max(d => d.priority) : 0;
        }

        public void Add(Career entry)
        {
            db.Careers.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.Careers.Where(d => !d.isDeleted);
        ...
        //{
        //    db.Careers.DeleteOnSubmit(db.Careers.FirstOrDefault(d=>d.id == id));
        //}
