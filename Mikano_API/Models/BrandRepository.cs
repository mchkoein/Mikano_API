...
namespace Mikano_API.Models
{
    ...
    #region Methods
    public class BrandRepository : SharedRepository
    {
        public dblinqDataContext db = new dblinqDataContext();

        public IQueryable<Brand> GetAll()
        {
            return db.Brands.Where(d => !d.isDeleted).OrderByDescending(d => d.priority);
        }

        public IQueryable<Brand> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public Brand GetNextEntry(Brand entry)
        {
        ...
        {
            return db.Brands.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }

        //public Brand GetByUrlTitle(string title)
        //{
        //    return db.Brands.FirstOrDefault(d => !d.isDeleted && d.title.Replace("<br/>", "-").Replace("<br>", "-").Replace("!", "").Replace(":", "-").Replace("'", "").Replace(" ", "-").Replace("&", "").Replace("?", "").Replace("/", "-").Replace("*", "-").Replace(".", "").Replace("--", "-").ToLower() == title.ToLower());
        //}

        //public Brand GetByNativeTitle(string title)
        //{
        //    return db.Brands.FirstOrDefault(d => !d.isDeleted && d.title.ToLower() == title.ToLower());
        //}

        public int GetMaxPriority()
        {
            return db.Brands.Any() ? db.Brands.Max(d => d.priority) : 0;
        }

        public void Add(Brand entry)
        {
            db.Brands.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.Brands.Where(d => !d.isDeleted);
        ...
        //{
        //    db.Brands.DeleteOnSubmit(db.Brands.FirstOrDefault(d=>d.id == id));
        //}
