...
{
    #region Models
    [MetadataType(typeof(BrandItemValidation))]
    public partial class BrandItem
    {
        public int parentId { get; set; }
    }

    public class BrandItemValidation
    {
    ...
    #region Methods
    public class BrandItemRepository : SharedRepository
    {
        public dblinqDataContext db = new dblinqDataContext();

        public IQueryable<BrandItem> GetAll(int? parentId = null)
        {
            return db.BrandItems.Where(d => !d.isDeleted && d.parentId == parentId).OrderByDescending(d => d.priority);
        }

        public IQueryable<BrandItem> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public BrandItem GetNextEntry(BrandItem entry)
        {
        ...
        public BrandItem GetById(int id)
        {
            return db.BrandItems.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }

        //public BrandItem GetByUrlTitle(string title)
        //{
        //    return db.BrandItems.FirstOrDefault(d => !d.isDeleted && d.title.Replace("<br/>", "-").Replace("<br>", "-").Replace("!", "").Replace(":", "-").Replace("'", "").Replace(" ", "-").Replace("&", "").Replace("?", "").Replace("/", "-").Replace("*", "-").Replace(".", "").Replace("--", "-").ToLower() == title.ToLower());
        //}

        //public BrandItem GetByNativeTitle(string title)
        //{
        //    return db.BrandItems.FirstOrDefault(d => !d.isDeleted && d.title.ToLower() == title.ToLower());
        //}

        public int GetMaxPriority()
        {
            return db.BrandItems.Any() ? db.BrandItems.Max(d => d.priority) : 0;
        }

        public void Add(BrandItem entry)
        {
            db.BrandItems.InsertOnSubmit(entry);
        }

        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.BrandItems.Where(d => !d.isDeleted);
        ...
        //{
        //    db.BrandItems.DeleteOnSubmit(db.BrandItems.FirstOrDefault(d=>d.id == id));
        //}
