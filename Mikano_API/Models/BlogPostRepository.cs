...
namespace Mikano_API.Models
{
    ...
    #region Methods
    public class BlogPostRepository : SharedRepository
    {
        public dblinqDataContext db = new dblinqDataContext();

        public IQueryable<BlogPost> GetAll()
        {
            return db.BlogPosts.Where(d => !d.isDeleted).OrderByDescending(d => d.priority);
        }

        public IQueryable<BlogPost> GetAllIsPublished()
        {
            return GetAll().Where(d => d.isPublished);
        }

        public BlogPost GetNextEntry(BlogPost entry)
        {
        ...
        {
            return db.BlogPosts.FirstOrDefault(d => !d.isDeleted && d.id == id);
        }

        //public BlogPost GetByUrlTitle(string title)
        //{
        //    return db.BlogPosts.FirstOrDefault(d => !d.isDeleted && d.title.Replace("<br/>", "-").Replace("<br>", "-").Replace("!", "").Replace(":", "-").Replace("'", "").Replace(" ", "-").Replace("&", "").Replace("?", "").Replace("/", "-").Replace("*", "-").Replace(".", "").Replace("--", "-").ToLower() == title.ToLower());
        //}

        //public BlogPost GetByNativeTitle(string title)
        //{
        //    return db.BlogPosts.FirstOrDefault(d => !d.isDeleted && d.title.ToLower() == title.ToLower());
        //}

        public int GetMaxPriority()
        {
            return db.BlogPosts.Any() ? db.BlogPosts.Max(d => d.priority) : 0;
        }

        public void Add(BlogPost entry)
        {
            db.BlogPosts.InsertOnSubmit(entry);
        }
        public void DeleteTagsByBlogPost(int id)
        {
            db.PostTags.DeleteAllOnSubmit(db.PostTags.Where(d => d.post_id == id));
        }
        public string SortGrid(int newIndex, int oldIndex, int id)
        {
            var entry = GetById(id);
            var allData = db.BlogPosts.Where(d => !d.isDeleted);
        ...
        //{
        //    db.BlogPosts.DeleteOnSubmit(db.BlogPosts.FirstOrDefault(d=>d.id == id));
        //}
        ...
        #endregion
        public void DeleteRelatedMedias(BlogPost entry)
        {
            db.PostMedias.DeleteAllOnSubmit(db.PostMedias.Where(d => d.post_id == entry.id));
        }
    }
    #endregion
}